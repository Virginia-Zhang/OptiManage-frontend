...
const addMarketing = () => {
	...
	addMarketingFormRef.value.validate(async valid => {
		...
		const startTime = formatTime(addMarketingForm.value.startTime)
		const endTime = formatTime(addMarketingForm.value.endTime)
		addMarketingForm.value.startTime = startTime
		addMarketingForm.value.endTime = endTime
		...
		switch (currencyUnit.value) {
			case "RMB":
				costRmb = Number(addMarketingForm.value.cost)
				addMarketingForm.value.costRmb = costRmb
				break
			case "JPY":
				costJpy = Number(addMarketingForm.value.cost)
				addMarketingForm.value.costJpy = costJpy
				break
			case "USD":
				costUsd = Number(addMarketingForm.value.cost)
				addMarketingForm.value.costUsd = costUsd
				break
		}
		// Delete cost attribute
		delete addMarketingForm.value.cost
		// Open loading
...
const handleCancel = formEl => {
	...
	currencyUnit.value = ""
	// Reset costs
	if (addMarketingForm.value.costRmb) delete addMarketingForm.value.costRmb
	if (addMarketingForm.value.costUsd) delete addMarketingForm.value.costUsd
	if (addMarketingForm.value.costJpy) delete addMarketingForm.value.costJpy
	// Close pop-up window
	showAddMarketingDialog()
}
...
const handleClose = done => {
	// Reset form data
	addMarketingFormRef.value.resetFields()
	// Reset currency unit
	currencyUnit.value = ""
	// Reset costs
	if (addMarketingForm.value.costRmb) delete addMarketingForm.value.costRmb
